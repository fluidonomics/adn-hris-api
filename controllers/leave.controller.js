...
let functions = {
                ...
                leaveMasterData.forEach(f => {
                    ...
                    if (currentData.length > 0) {
                        ...
                        finalResponse.push({
                            ...
                            createdAt: f.createdAt, updatedAt: f.updatedAt, status: f.status,
                            createdByName: currentData[0].leavedetails.createdByName, updatedByName: currentData[0].leavedetails.updatedByName,
    ...
    getLeaveDetailsById: (req, res) => {
        ...
        async.waterfall([
                 ...
                 leaveMaster.find({_id: parseInt(req.query.id)}, function(err, result){
                     done(err, result[0]);
                 })
            },
            (response, done) => {
                let leaveApplyId = response.leaveAppliedId;
                LeaveApply.aggregate([
                    queryObj,
                    { "$match": { "isDeleted": false, "_id":leaveApplyId } },
                    {
                        "$lookup": {
                            "from": "employeedetails",
                            "localField": "emp_id",
